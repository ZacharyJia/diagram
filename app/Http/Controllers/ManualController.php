...
class ManualController extends BaseController
{
    ...
    {
        $autos = Manual::distinct()->where('id', '!=', '0')->get(['id']);
    ...
    public function getData(Request $request)
    {
        ...
        $datas = Manual::where('id', $number)->get();
        if(count($datas) == 0) {
            return "null";
        }
...
//            $arr['time'][] = $data->time;
    ...
    {
        $ids = Input::all(['input_id']);
        return view('addPoint')->with('ids', $ids);
    }
